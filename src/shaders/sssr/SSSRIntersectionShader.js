    ...
    fragmentShader: /* glsl */ `
        ...
        out vec3 lastP) 
		{
			...
			for(int i = 0; i < steps; i++) {
				...
				// out_SSRColor =  vec4( p, 1.0 );
				// out_SSRColor =  vec4( projP );
				// out_SSRColor =  vec4( pUv, 0.0, 1.0 );
			...
			// out_SSRColor =  vec4( possibleIntersection, abs(depthAtP2 - lastRecordedDepthBuffThatIntersected) < maxIntersectionDepthDistance, intersected, 1.0 );
			return intersected;
		}
		...
		void main() {
			...
			float baseF0    = 0.08;
			baseF0 = mix(baseF0, 0.4, uMetallicMultiplier);

			roughness = roughness * roughness;
			// roughness = sqrt(roughness);

			// roughness = mix(roughness, 0.05, 1.0 - sqrt(uRoughnessMultiplier));
			roughness = mix(roughness, 0.05, sqrt(uRoughnessMultiplier));

			if (roughness > 0.5 || depth == 0.0) {
			...
			for(uint s = 0u; s < rays; s++) {
				vec2 hash = Hammersley16(s, rays, random);
				vec3 wm;
            	// vec3 reflDir = SampleBRDF(viewDir, norm, s, roughness, wm);
            	vec3 reflDir = ImportanceSamplingGGX(viewDir, norm, hash, roughness, wm);
				...
				if(dot(reflDir, norm) < 0.0) {
					if(effectiveSamples > 1u) {
						...
						reflDir = ImportanceSamplingGGX(viewDir, norm, hash, roughness, wm);
					}
				}
